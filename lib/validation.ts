...
export const dayWindowSchema = z
  ...
  .refine((w) => timeToMin(w.wakeTime) < timeToMin(w.sleepTime), "wake must be before sleep")
  .refine((w) => timeToMin(w.workStartTime) <= timeToMin(w.workEndTime), "work start must be <= work end");
...
export const settingsSchema = z.object({
  ...
  agentProvider: z.enum(["openrouter", "agy", "fake", "claude"]).optional(),
